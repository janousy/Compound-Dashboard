...
const supplyContractAddress = '0xE8a6f04DEF6d9fa9F02982b8E1d8Eea835fB7668';
const supplyAbi = require('../artifacts/contracts/CompoundSupply.sol/CompoundSupply.json').abi;
...
const main = async function() {
  ...
  let supplyResult = await supplyContract.methods.supplyEthToCompound(
  ...
  let cTokenBalance = await compoundCEthContract.methods.
    balanceOf(supplyContractAddress).call();
  ...
  let redeemResult = await supplyContract.methods.redeemCEth(
    amount,
    redeemType,
    compoundCEthContractAddress
  ).send({
  ...
  cTokenBalance = await compoundCEthContract.methods.balanceOf(supplyContractAddress).call();
  cTokenBalance = cTokenBalance / 1e8;
  console.log("MyContract's cETH Token Balance:", cTokenBalance);

  ethBalance = await web3.eth.getBalance(supplyContractAddress);
